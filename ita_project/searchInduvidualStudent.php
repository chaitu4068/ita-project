	...
	<div id="content">
		...
		<?php
		   if(!($connect = mysqli_connect("localhost","root","","ita_project")))
			...
			{
			$query = "select * from student where uname = '" . $_POST["username"] . "'";
			if(!($result = mysqli_query($connect,$query)))
				{
					header("Location:team_alpha_admin_view_induvidual_student.php");
				}
			...
			{
			$query = "select * from student where rollnumber = '" . $_POST["roll_no"] . "'";
			if(!($result = mysqli_query($connect,$query)))
				{
					header("Location:team_alpha_admin_view_induvidual_student.php");
				}
