...
<body>
	...
	<div id="content">
		...
		<select id="numWeights" name = "noofcompos">
		...
		<p><a href = "viewExistingPlan.php" target = "_blank">View Existing Plan</a></p>
		</form>
		
		<?php
//print("helo wolrd");
$noofcompos = $_POST['noofcompos'];
settype($noofcompos,"integer");

//print("No ofelements in post is : " . $count);
//print($_GET['course']);
print ("<form method = \"POST\" action = \"addToWeightageTable.php?course=".$_GET['course']."\">");
			print ("<table>");
			print ("<tr>");
			print ("<th>Type</th>");
			print ("<th>Total</th>");
			...
			print ("</tr>");
		for($i = 0; $i < $noofcompos; $i++)
			{
			$nam = "comp" . $i;
            $tot = "total" . $i;			
			$wei = "wtg" . $i;  
			print ("<tr>");
			print ("<td><input type='text' name = '" . $nam . "' /></td>");
			print ("<td><input type='text' name = '" . $tot . "' /></td>");
			print ("<td><input type='text' name = '" . $wei . "'/></td>");			
			print ("</tr>");
			}
			print ("<table>");
