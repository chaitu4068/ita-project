...
<body>
	...
	<div id="content">
		...
		window.onload = getReferences();
		function getReferences()
		{
		var opts = document.getElementById('numReference').options;
		print '<form name="reference">';
			print '<tr>';
			print '<th>Reference Number</th>';
		...
		for(var i = 0, i < opts; i++)
			{
			print '<tr>';
			print '<td><input type="text" name="refnum$i" /></td>';
			print '<td><input type="text" name="refname$i" /></td>';
			print '<td><input type="text" name="reflink$i" /></td>';			
			print '</tr>';
			}
		print "<h3 id='bold'>Books</h3>";
		print '<textarea name="reftextarea"></textarea>';
		print '<input type="Submit" />';
		print '</form>';
		}
		</script>
		<form method = "POST" action = "team_alpha_grades_ref_ui.php?course=<?php print($_GET['course']);?>" >
		<h3>Please Enter the number of References</h3>
		<select id="numReference" name = "noofrefs">
